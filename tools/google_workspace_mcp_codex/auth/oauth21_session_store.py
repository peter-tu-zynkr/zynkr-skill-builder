...
class OAuth21SessionStore:
    ...
    def __init__(self):
        ...
        self._lock = RLock()
        self._oauth_state_file = os.path.join(
            os.path.expanduser("~"), ".google_workspace_mcp", "oauth_states.json"
        )

    def _serialize_oauth_states_locked(self) -> Dict[str, Dict[str, Any]]:
        serialized: Dict[str, Dict[str, Any]] = {}
        for state, data in self._oauth_states.items():
            serialized[state] = {
                "session_id": data.get("session_id"),
                "created_at": (
                    data["created_at"].isoformat() if data.get("created_at") else None
                ),
                "expires_at": (
    ...
    def _load_oauth_states_from_disk_locked(self) -> None:
        try:
            with open(self._oauth_state_file, "r", encoding="utf-8") as fh:
                raw_states = json.load(fh)
        except FileNotFoundError:
            return
        except Exception as exc:
            logger.warning("Failed to read OAuth state file %s: %s", self._oauth_state_file, exc)
            return
        ...
        for state, data in raw_states.items():
            try:
                ...
                logger.warning("Skipping invalid OAuth state entry %s: %s", state[:8], exc)

        self._oauth_states = loaded_states

    def _persist_oauth_states_locked(self) -> None:
        os.makedirs(os.path.dirname(self._oauth_state_file), exist_ok=True)
        temp_path = f"{self._oauth_state_file}.tmp"
        with open(temp_path, "w", encoding="utf-8") as fh:
            json.dump(self._serialize_oauth_states_locked(), fh)
        os.replace(temp_path, self._oauth_state_file)
    ...
    def _cleanup_expired_oauth_states_locked(self):
        ...
        self._persist_oauth_states_locked()
