#!/usr/bin/env python3
# Copied from skills/3-operations/project-status-update/scripts/ @ e35a1f57 (2026-08-03) for consult-status-report — re-sync if the source renderer changes.
...
# Fork divergence (the ONLY functional edit vs the source renderer): the source
# footer named its tracker-Sheet tab as the data source. This fork's email is
# CLIENT-facing and reads from the CRM + the engagement's Drive folder, so the
# footer stays generic — internal tool/Sheet names must never reach a client.
FOOTER_NOTE = "本週報由 Zynkr 顧問團隊整理提供"
