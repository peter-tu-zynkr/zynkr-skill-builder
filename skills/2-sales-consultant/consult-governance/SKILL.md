...
`mcp__zynkr__get_deal` by id, or `mcp__zynkr__list_deals(search="<company>")`
by name, folder via the `專案資料夾：<url>` backlink in the deal notes. If either half
...
assuming it. Then `mcp__zynkr__list_deals(limit=100)` and drop the rows whose
stage is 成交/未成交 (`won` / `lost`) — the open portfolio is what this step
inventories.

Two things to know about that call. It returns the newest 100 by activity and
is scoped to your own workspace, so if the count comes back at exactly 100,
say so in the report and treat the inventory as partial rather than complete —
a governance sweep that silently saw only part of the portfolio is worse than
one that admits the edge. And `list_deals` does not return `notes`; for the
notes-recency finding, fetch the deals you are actually judging with
`mcp__zynkr__get_deal`, which does.

For the activity pulse, use `last_activity_at` from the same rows. This skill
writes nothing, so every call here is a read.
