...
> **Known, accepted, unfixed: the Doc grows without bound.** Every writer here is insert-only
> and the scaffold copies the newest section *verbatim*, so each section inherits every
> `〔自動彙整〕` block ever written. Audited 2026-09-15: 34 sections / 5,325 lines, the newest
> section holding 22 blocks of which only 4 were that week's, one week's blocks present 16 times
> over, sections grown 130 → 292 lines in four weeks. An automated archive was built and
> **rolled back on 2026-09-15** (`SKB-029`) — see that spec before proposing one again. Trimming
> the Doc is a human act, at a Thursday meeting.
...
- **Nothing trims the Doc.** It grows by a section a week, and each section carries every
  auto-block before it. See the note under the cadence table, and `SKB-029` for the archive
  that was built, failed its rehearsal and was rolled back.
