> **Provenance:** copied verbatim from `zynkr-gm/references/derived-state-rules.md` (SKB-006, sheetId 0.02) on 2026-08-17 while that skill was still on its build branch. `zynkr-gm` is the OWNER of these rules; `planning-tracker-sync` only consumes them. If zynkr-gm changes its thresholds or state names, re-copy this file — never edit it here.
