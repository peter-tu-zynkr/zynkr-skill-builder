...
- Google account for every `google-workspace` call: `peter_tu@zynkr.ai` (user may
  ...
  returns a local path; HTTP mode a 1-hour URL to fetch) → the `Read` tool views the
  image; HEIC → `sips -s format jpeg <in>.HEIC --out <out>.jpg` first (macOS).
...
- **Transcript** — Doc ID → `get_doc_as_markdown`; pasted text → as is; Fireflies
  ...
  after:<date>` → `get_gmail_message_content`. Note whether it is punctuated /
  speaker-labelled (the July Doc in sources §A is one unpunctuated block — expect that).
- **Photos** — Drive IDs → `get_drive_file_download_url` → local file (fetch into a temp
  dir if HTTP mode) → convert HEIC → `Read`. If a board is dense, crop one image per
  column with PIL (`Image.open(f).crop((l, t, r, b))` — offset-exact; `sips -c` only
  crops from the centre, use it as a last resort) and read each strip at full
  resolution — that is what keeps 判讀信心 honest. Record file names + boards / columns /
  pen colours seen.
...
layout labels, strike-throughs included), pen colour as seen, 判讀信心 高/中/低, 備註 with
candidate readings for 中/低 and `非工作項` for layout marks. If more than one board,
prefix 白板欄位 with the board name (`to-be · Sales`, `as-is · 第 3 欄`). Print the total
...
3. Build `④` (schemas §3): 窮盡性 — all eight L1 with 項目數 · 涵蓋程度 · 說明; 互斥性 —
   ...
   quote what should have been there — the quote is the 說明.
...
1. **Segment** the block into speaker turns on cues (「OK 那換…」「我這邊…」「換你」
...
4. Any explicit 重要/緊急/owner statements from Part 2 (looking forward) go into a side
   list `逐字稿裡的決策` for planning-tracker-builder — not into the item list.
...
note, 分頁導覽. Same row-plan-then-write discipline.
...
acceptable when the user says "just draft"), pack §5 shape via schemas §7: TL;DR →
...
the item list lives (chat / `⑤ 正規化清單` / `.md` ID) · draft ID + recipients · an
...
- Handwriting is read by a vision pass, not OCR — dense or oblique photos lower 判讀信心;
  the answer is the `請與會者確認` list, not a guess. Crop per column when in doubt.
