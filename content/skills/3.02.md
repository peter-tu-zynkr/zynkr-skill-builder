...
input: >-
  ...
  specifies); the KB is the Zynkr platform 知識庫 (platform.zynkr.ai/kb) — sections
  + fact/qa cards, accessed...
process: >-
  Pull resolved, not-yet-ingested threads → confirm they are learnable and
  extract the question + Peter's answer (PII-stripped) → draft cards through
  three lenses (intent classification / target section / fact-vs-qa structuring)
  → propose to Peter for approval...
output: >-
  After Peter's approval, cards are written into the matching platform-KB
  section; a new intent creates a new section via create_kb_section; the touched
  sections and the JSON...
ipoProvenance:
  input: frontmatter
  process: frontmatter
  output: frontmatter
