...
input: >-
  ...
  Support knowledge base` 資料夾內的『巢狀』文件組（00 INDEX & Retrieval Map 路由 → 各 section
  doc + 01 Core Facts）
process: >-
  拉出已回覆、未匯入的 threads → 確認可學習並抽取 Q＋Peter 的答覆（去 PII）→ 三鏡頭（意圖分類／落點 section
  doc／結構化）擬條目 → 提案給 Peter 核可 → 經 INDEX 路由寫入對應 section doc（fact 寫 01 Core Facts）→
  標記 KMS-ingested
output: >-
  經 Peter 核可後寫入巢狀 KB 對應 section doc 的條目；新意圖則新建 section doc 並在 INDEX 註冊；來源 thread
  標記 KMS-ingested；結束給一份新增／更新／略過摘要
ipoProvenance:
  input: frontmatter
  process: frontmatter
  output: frontmatter
