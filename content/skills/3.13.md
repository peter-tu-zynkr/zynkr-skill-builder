...
input: >-
  Gmail 中未回覆的 Support／Inbound-Sales／website-form threads；KB 為 `[3.2] Support
  knowledge base` 資料夾內的『巢狀』文件組（00 INDEX & Retrieval Map 為路由入口）
process: >-
  拉未回覆 queue → 讀全文＋偵測未回覆＋判斷語言 → 辨識 intent → 經 INDEX 路由到對應 section doc＋01 Core
  Facts 找答案 → 有答案起草正式回信／無答案起 holding reply＋[[NEEDS PETER]] 並通知 Peter → Peter
  補上答案後，把該 thread 交棒給 /zynkr-kms 寫回 KB
output: >-
  每封 thread 一份 Gmail 草稿；一份摘要（已草稿／需要 Peter／已跳過）；對已由 Peter 補答的 thread，觸發
  /zynkr-kms 交棒
ipoProvenance:
  input: frontmatter
  process: frontmatter
