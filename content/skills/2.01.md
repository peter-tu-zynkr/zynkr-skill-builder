...
description: >-
  ...
  tailored 陌生開發 strategy, Hot Lead flag, and Well-Known firm flag. Operates in
  BATCH on a WHOLE survey sheet and writes back only new columns — it creates no
  CRM records and drafts no email. For ONE person's row — or a single DM,
  website enquiry, or business card — hand that signal to sales-outbound
  instead, which creates the contact + deal and drafts the reply. Natural
  pairing: run this skill to triage the sheet, then sales-outbound on each Hot
  Lead row.
input: >-
