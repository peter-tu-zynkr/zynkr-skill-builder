...
function inferPlatform(record: SheetRecord): SkillPlatform {
  const values = [
    record["GPT"] ? "gpt" : null,
...
function toSkill(record: SheetRecord): Skill {
...
export class GoogleSheetsSkillProvider implements SkillDataProvider {
  async listSkills(): Promise<Skill[]> {
    ...
    return dataRows
      .map((row) =>
        Object.fromEntries(headerRow.map((header, index) => [header, String(row[index] ?? "").trim()]))
      )
